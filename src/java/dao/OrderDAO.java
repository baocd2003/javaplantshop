...
public class OrderDAO {
    ...
    public static ArrayList<Order> getOrdersByID(String accid) throws Exception {
        ArrayList<Order> list = new ArrayList<>();
        Connection cn = DBUtils.makeConnection();
        if (cn != null) {
            String sql = "select OrderID,OrdDate,shipdate,Orders.status,Orders.AccID\n"
    ...
    public static ArrayList<Order> getOrdersByDate(String dateFrom, String dateTo) {
        ...
        try {
            ...
            if (cn != null) {
                String sql = "select OrderID,OrdDate,shipDate,status,AccID\n"
                        + "from Orders where Orders.OrdDate between ? and ?";
                PreparedStatement pst = cn.prepareStatement(sql);
                Date ordDate1 = Date.valueOf(dateFrom);
                Date ordDate2 = Date.valueOf(dateTo);
                pst.setDate(1, ordDate1);
                pst.setDate(2, ordDate2);
